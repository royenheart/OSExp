...
int main(int argc, char *argv[]) {
    ...
    // 得到根目录的数据块索引表
    father_index_table_blocknr =
        inode_table[file_father_inode].index_table_blocknr;
    read_dir_index_table(fp_xtfs, dir_index_table,
                         father_index_table_blocknr * BLOCK_SIZE);
    ...
    for (i = 0; i < dir_num; i++) {
        ...
        child_in_father_index =
            find_dir_index_table(dir_names[i], dir_index_table, DIR_FILE);
    ...
    int file_index;
    file_index =
        find_dir_index_table(dir_names[dir_num], dir_index_table, type);
    ...
    refather_index_table_blocknr =
        inode_table[refile_father_inode].index_table_blocknr;
    read_dir_index_table(fp_xtfs, redir_index_table,
                         refather_index_table_blocknr * BLOCK_SIZE);

    for (i = 0; i <= dir_renum; i++) {
        int child_in_father_index;
        child_in_father_index =
            find_dir_index_table(dir_renames[i], redir_index_table, DIR_FILE);
        // 找到和未找到的处理
        if (child_in_father_index == NOT_FOUND) {
    ...
    int refile_index;
    refile_index =
        find_dir_index_table(dir_names[dir_num], redir_index_table, type);
    if (refile_index != NOT_FOUND) {
        printf(
            "The target address already has a file/folder named %s with "
            "type %d!\n",
            argv[1], type);
        fclose(fp_xtfs);
        xtfs_exit(EXIT_FAILURE);
    ...
    // 在目标目录下进行剪切操作
    get_empty_dir_index(redir_index_table, dir_names[dir_num], type,
                        file_inode);

    // 全部结束后写入文件系统分区
    write_file(fp_xtfs, father_index_table_blocknr * BLOCK_SIZE,
               (char *)dir_index_table, CATALOG_TABLE_SIZE * sizeof(CATALOG));
    write_file(fp_xtfs, refather_index_table_blocknr * BLOCK_SIZE,
               (char *)redir_index_table, CATALOG_TABLE_SIZE * sizeof(CATALOG));
