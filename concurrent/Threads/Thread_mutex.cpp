...
void *thread(void *attr) {
    ...
    while (food > 5) {
        ...
        food -= eatNum;
        cout << myname << " Eat " << eatNum << " food." << endl
             << "Now left " << food << " food" << endl;
        pthread_mutex_unlock(&mutex);
    }
...
int main(int argc, char *argv[]) {
    void *tret1, *tret2;
    // Due to threads running concurrently, can't assign their return to the
    // same vars will Occur some erorrs
