...
TEST_F(FILE_TYPE, NORMAL_EXE) { ASSERT_EQ(get_file_type(EXE_FILE), EXE_FILE); }
...
TEST_F(FILE_TYPE, SPEC_TEXT) {
    ...
    ASSERT_EQ(get_file_type(TEXT_FILE | CIPHER | ZIP),
              TEXT_FILE | CIPHER | ZIP);
}
...
TEST_F(FILE_TYPE, SPEC_UNKNOWN) {
    ...
    ASSERT_EQ(get_file_type(UNKNOWN_FILE | CIPHER | ZIP),
              UNKNOWN_FILE | CIPHER | ZIP);
    ASSERT_EQ(get_file_type((UNKNOWN_FILE + 1) | CIPHER),
              (UNKNOWN_FILE) | CIPHER);
    ASSERT_EQ(get_file_type((UNKNOWN_FILE + 1) | ZIP), (UNKNOWN_FILE) | ZIP);
    ASSERT_EQ(get_file_type((UNKNOWN_FILE + 1) | CIPHER | ZIP),
              (UNKNOWN_FILE) | CIPHER | ZIP);
}
