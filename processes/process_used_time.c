...
int main(int argc, char *argv[]) {
    ...
    clock_t start, end;
    // struct tms 也用于衡量 CPU 时间，但是一个结构体类型，用于表示程序在用户、系统模式下分别使用的 CPU 时间，以及程序在开始运行后经历的时钟数，这里声明程序计算开始和结束使用的 CPU 时间的变量
    // 该结构体有 4 个成员变量：
    // 1. tms_utime：表示程序在用户模式下所使用的CPU时间。
    // 2. tms_stime：表示程序在系统模式下所使用的CPU时间。
    // 3. tms_cutime：表示由当前进程和它的子进程在用户模式下所使用的CPU时间之和。
    // 4. tms_cstime：表示由当前进程和它的子进程在系统模式下所使用的CPU时间之和。
    struct tms t_start, t_end;
    // 标准 C 库中，times 函数用于获取一个 struct tms 类型的值（参数中传指针填充），填充不同模式下使用的 CPU 时间，并返回进程启动后经历的时钟数（真实时间）。
    start = times(&t_start);
    #ifdef STRESS
    // fork 出子进程，也进行密集型计算
    pid_t pid = fork();
    ...
    int status;
    waitpid(-1, &status, 0);
    // 获取计算结束时进程计算经历的时钟数（end），并填充不同模式下使用的 CPU 时间（t_end）
    end = times(&t_end);
