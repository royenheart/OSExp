...
class FileSystem {
   private:
    ...
    int getCurDirIndex() { return getInodeIndex(cur_dir, "dir"); }
   ...
   public:
    ...
    const std::string &currentFolder() { return cur_dir; }
    ...
    void touch(const std::string &name) {
        ...
        for (int inode_index : dir_inode.pointer) {
            ...
            if (target_inode.name == name && target_inode.type == "f") {
                ...
                std::cout << "文件存在, 更新其创建日期为： "
                          << std::asctime(std::localtime(&mtime));
                return;
            }
        }
    ...
    void ls(const std::string &target = "") {
        ...
        if (ii == -1) {
            std::cout << "未找到目标目录 inode 信息：" << target_dir
                      << std::endl;
            return;
            ...
            for (int inode_index : dir_inode.pointer) {
                ...
                if (now.ondelete == false) {
                    std::cout << now.name << "\t" << now.type << "\t"
                              << now.permission << "\t" << now.size << "\t"
                              << std::asctime(std::localtime(&now.mtime));
                }
            }
        }
    }
    ...
    void saveToFile(const std::string &filename) {
        ...
        for (const auto &inode : inode_table) {
            ...
            file.write(reinterpret_cast<const char *>(&nameSize),
                       sizeof(size_t));
            file.write(inode.name.c_str(), nameSize);

            file.write(reinterpret_cast<const char *>(&typeSize),
                       sizeof(size_t));
            file.write(inode.type.c_str(), typeSize);

            file.write(reinterpret_cast<const char *>(&permissionSize),
                       sizeof(size_t));
            file.write(inode.permission.c_str(), permissionSize);

            // 保存其他成员
            file.write(reinterpret_cast<const char *>(&inode.size),
                       sizeof(int));
            file.write(reinterpret_cast<const char *>(&inode.ondelete),
                       sizeof(bool));
            file.write(reinterpret_cast<const char *>(&inode.mtime),
                       sizeof(time_t));

            // 保存指针集合
            size_t pointerSize = inode.pointer.size();
            file.write(reinterpret_cast<const char *>(&pointerSize),
                       sizeof(size_t));
    ...
    void loadFromFile(const std::string &filename) {
        ...
        while (true) {
            ...
            if (!file.read(reinterpret_cast<char *>(&nameSize),
                           sizeof(size_t))) {
                break;
            }
            ...
            if (!file.read(reinterpret_cast<char *>(&typeSize),
                           sizeof(size_t))) {
                break;
            }
            ...
            if (!file.read(reinterpret_cast<char *>(&permissionSize),
                           sizeof(size_t))) {
                break;
            }
            ...
            // 构造Inode对象并添加到数据集合
            inode_table.push_back(
                {name, type, permission, size, ondelete, mtime, pointer});
        }
