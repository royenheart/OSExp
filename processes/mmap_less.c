...
int main(int argc, char* argv[]) {
    ...
    if (addr == MAP_FAILED) {
        ...
        switch (errno) {
            case EBADE:
                ...
                break;
            case EACCES:
                perror("Incorrect access permissions. If it is MAP_PRIVATE, the file must be readable, and if MAP_SHARED is used, PROT_WRITE is required and the file must be able to be written.");
                break;
            case EINVAL:
    ...
    while (remaining_bytes > 0) {
        // 每次读取页面大小或剩下的页面
        size_t page_bytes = remaining_bytes < PAGE_SIZE ? remaining_bytes : PAGE_SIZE;
        // 打印输出
