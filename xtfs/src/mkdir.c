...
int main(int argc, char *argv[]) {
    ...
    read_dir_index_table(fp_xtfs, index_table,
    ...
    for (i = 0; i <= dir_num; i++) {
        ...
        child_in_father_index =
            find_dir_index_table(dirnames[i], index_table, DIR_FILE);
        // 找到和未找到的处理
        if (child_in_father_index == NOT_FOUND) {
            ...
            child_in_father_index = get_empty_dir_index(
                index_table, dirnames[i], DIR_FILE, new_inode);
            // 由于目录的 index table
            // 块处于和普通文件共享的数据区，且目录需要从此块读取数据，因此每次申请到新的
            // index table 空间，都需要将其覆盖，防止之前的数据的影响
            ...
            write_file_with_blank(
                fp_xtfs, father_index_table_blocknr * BLOCK_SIZE,
                (char *)index_table, CATALOG_TABLE_SIZE * sizeof(CATALOG));
            // 进行迭代
            ...
            father_index_table_blocknr =
                inode_table[curr_inode].index_table_blocknr;
            // 进行迭代
            read_dir_index_table(fp_xtfs, index_table,
                                 father_index_table_blocknr * BLOCK_SIZE);
