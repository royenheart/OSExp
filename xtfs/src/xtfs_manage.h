 ...
 * @return short blocknr
 * 返回在数据块位图的第几个字节的第几位（表示具体哪一块，512 * 8 = 4096）
 */
 ...
 * @deprecated 此函数只是申请了 index_table 需要的空间就直接向文件系统写入了
 * index_table，不符合空间全部申请再读写的要求。
 */
 ...
 * @deprecated 目录添加后，只根据文件名在 inode 表中查找的方法已经失效。inode
 * 表中允许出现相同名字甚至类型相同的表项。
...
/**
 * @brief 从目录的第一个 index_table 开始查找空余位置，找到则将文件数据写入
 * index_table
 *
 ...
 *
 * @param buffer
 * 待写入文件系统的Buffer字节数组，默认为BLOCK_SIZE，后面可能需要适应更改
...
/**
 * @brief
 * 在装载好文件写入的必要参数后，根据文件大小和文件类型将文件内容和对应的index_table全部先申请后再写入文件系统。
 *
